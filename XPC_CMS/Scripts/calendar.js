...
var monthName = {
    en : new Array('Tháng Giêng','Tháng Hai','Tháng Ba','Tháng Tư','Tháng Năm','Tháng Sáu','Tháng Bảy','Tháng Tám','Tháng Chín','Tháng Mười','Tháng Mười Một','Tháng Mười Hai'),
...
if (startAt==0) {
    dayName = {
        en : new Array('T2','T3','T4','T5','T6','T7','CN'),
...
function closeAction(bln)
{
	//TODO:
}
function closeCalendar(bln) {
    var timed = new Date();
    ...
    var objHidden=null;
    if(document.body.objHidden)
    {
		objHidden = document.body.objHidden;
    }
    else
    {
		objHidden=document.createElement("input");
		objHidden.name="myCalendarHidden";
		objHidden.id="myCalendarHidden";    
		objHidden.type="hidden";
		ctlToPlaceValue.parentNode.appendChild(objHidden);
    }
    objHidden.value=ctlToPlaceValue.value ;  
    closeAction(bln);  
...
function constructMonth() {
    ...
    if (!monthConstructed) {
        ...
        for (i=0; i<12; i++) {
            ...
            sHTML += '<tr><td nowrap id="m' + i + '" class="pcalList" onmouseover="this.style.backgroundColor=\'#909090\'" onmouseout="this.style.backgroundColor=\'\'" onclick="selectMonth(' + i + ');event.cancelBubble=true">&nbsp;' + sName + '&nbsp;</td></tr>';
        }
...
function VC_Calendar(ctr,bln)
{   
    if (bln) action = true; 
	showCalendar(ctr, ctr, "dd/mm/yyyy", null,1,-1,-1,true);	
}
...
function showCalendar(ctl, ctl2, format, lang, past, fx, fy, isClearInvalid) {
    ...
    if (showToday == 1) {
        document.getElementById('lblToday').innerHTML = '<font color="#000066">' + todayString[language] + ' <a style="color:#0000ff" onmousemove="window.status=\''+gotoString[language]+'\'" onmouseout="window.status=\'\'" title="'+gotoString[language]+'" style="'+styleAnchor+'" href="javascript:selectToday();">'+dayName[language][(today.getDay()-startAt==-1)?6:(today.getDay()-startAt)]+', ' + dateNow + ' ' + monthName[language][monthNow] + ' ' + yearNow + '</a></font>';
    }
    ...
    if (bPageLoaded) {
        if (crossobj.visibility == 'hidden') {
            ...
            if ((tokensChanged != 3) || isNaN(dateSelected) || isNaN(monthSelected) || isNaN(yearSelected) ||
                (monthSelected <0) || (dateSelected <=0) || (monthSelected > 11) || (dateSelected > 31))
            {
...
document.onkeypress = function hidecal1 (evt) {
    ...
    if (evt.keyCode == 27) hideCalendar();
}
